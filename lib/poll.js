...
var createPoll = function(request, reply) {
    ...
    redis.setnx(poll_id, JSON.stringify(poll_obj)).then(function(result) {
        ...
        if (result === 0 ) {
            ...
            // Generate empty array with 0's corresponding to each answer in the options array
            var answers = [];
            for (var i = 0; i < options.length; i++) {
                answers.push(0);
            }

            // Set this answers list in redis. So it should like something like poll:asdj3:answers where poll:asdj3:answers[0] =  number of people who voted for option 1
            redis.lpush(poll_id + ':answers', answers);
